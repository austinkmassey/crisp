...
function parse_current_index() {
    ...
    if [[ -z "$result_var" ]]; then
        echo "Error: result variable name is required for parse_current_index" >&2
        return 1
    fi

    declare -n index_data_ref="$result_var"
    index_data_ref=()  # Initialize the associative array
...
function update_index_data() {
    ...
    for file in "${pending_files_ref[@]}"; do
        ...
        index_data_ref["$file"]="- [$(basename "$file")](${relative_path})"
    done
}
